...
// visualization. A standard user cannot create new visualizations. An admin
// user may create visualizations for their group's authorized assets. A
// superuser may create visualizations for all assets.
func addHandler(ctx context.Context, s *state.State, a *auth.State, w http.ResponseWriter, r *http.Request) {
	...
	if request.Group == "" || len(request.Authorized) == 0 || request.Name == "" || request.Class == "" || request.DataIndex == "" {
	...
	// ensure provided asset actually exist for requesting user
	validAsset := false
